...
- two subtitles for media with separated settings (sync, color, position, font size,..) also can drag subtitles up and down over media.
- load/save settings into .conf/.ini file
- steady Audio Device: if it's ON => when audio output device changed pause media.
- custom cursor icon over media (to find cursor easier over movie) with your selected custom icon
